...
author:
  - Hannah Koenker^[Tropical Health LLP, hannah@trophealth.com, corresponding author]
  - E. Kuor Kumoji^[Johns Hopkins Center for Communication Programs, KuorKumoji@jhu.edu]
  ...
  - Cameron Taylor^[The DHS Program, ICF, & Medicine and Health Sciences University of Antwerp Belgium, Cameron.taylor@icf.com]
# address:
...
date: Revision of `r format(Sys.time(), '%d %B, %Y')`
...
output: 
  bookdown::word_document2:
    ...
    number_sections: no
    line_numbers: yes
    reference_docx: "../../../R Directory/Quantification/drdanholmes-ref-word-lineno.docx"
...
pnets <- read_dta("/Users/hannahkoenker/Dropbox/A DHS MIS Datasets/Analysis/Analysis ITN Indicators compiled/natl_DHSMISMICS_itn_indicators.dta") %>% 
  select(dataset, surveynameyear, country, year, survey, typeofsurvey, useitn, usenotenough, useenough, usetoomany, netsused, netsusedlb, netsusedub, netsusedinhhwithnotenoug, netsusedinhhwithjustrigh, netsusedinhhwithtoomany, urban, rural, pvalresidence) %>% 
  clean_names() %>%  
  drop_na(netsused) %>% 
  filter(typeofsurvey!="AP",
         dataset!="ZMHR15",
         dataset!="ZMHR18",
         dataset!="ZMHR21") # drop Zambia surveys that we aren't allowed to use for this. 
...
singlelist <- c("Senegal2011","Senegal2012","Senegal2014","Senegal2015","Senegal2016","Senegal2017","Senegal2018","Senegal2019","Senegal2021","Nigeria2010","Nigeria2015","Nigeria2018","Nigeria2021","Mali2021","Mozambique2018","Guinea2021","Kenya2020","Ghana2019","Madagascar2021")
...
The study objective is to use national population-based household survey data to characterize the reasons underlying non-use of ITNs. The goal of the analysis is to explore the reasons for not using an ITN during the previous night in relation to net supply at household level, and how these reasons vary by country and, where possible, by time. The research questions are:

1.	What proportion of nets were used the night prior to the survey?
1.	Of nets that went unused, what are the most frequently reported reasons for non-use, and how do reasons vary by country, household net supply, and residence?

# Methods



```{r bigmap, message=FALSE, warning=FALSE, include=FALSE}
bigmap <- pnets %>% 
...
#   st_crop(c(xmin= -19, ymin = -30, xmax = 161, ymax = 45))

afasia <- world %>% 
  filter(continent == "Africa" | continent=="Asia" | continent=="Oceania",!is.na(iso_a2), name_long!="New Zealand", name_long!="Fiji", name_long!="New Caledonia", name_long!="Vanuatu", name_long!="Solomon Islands")

bigmap <- afasia %>% 
  left_join(bigmap, by="iso_a2") %>% 
...
```{r, fig.cap = "Countries included in the analysis and number of DHS, MIS, and MICS surveys per country"}
mapa <- ggplot(bigmap) +
  geom_sf(
    data = bigmap,
    aes(fill = as.factor(nsurv)),
    color = "white",
    size = 0.5
  ) +
  theme_void() +
...
```{r smallmap, message=FALSE, fig.cap = "Countries with surveys containing questions about the reason nets were not used the previous night and number of DHS, MIS, and MICS surveys per country"}
smallmap <- nummis %>% 
  dplyr::select(country) %>% 
  mutate(cty=word(country)) %>% 
  count(cty) %>% 
  mutate(iso_a2=countrycode(cty, origin="country.name", destination="iso2c"))

africa = world %>%
  filter(continent == "Africa",!is.na(iso_a2)) 

smallmap <- africa %>% 
  left_join(smallmap, by="iso_a2") %>% 
  st_transform("+proj=aea +lat_1=20 +lat_2=-23 +lat_0=0 +lon_0=25")

mapb <- ggplot(smallmap) +
  geom_sf(
    data = smallmap,
...
For the first study objective, `r nsurv[1]` DHS, MIS, and Multiple Cluster Indicator Surveys (MICS) survey data collected since `r startyr` were downloaded with permission from dhsprogram.com and mics.unicef.org (Fig. 1A). Each dataset was reshaped to a long format to create a net file with details including its age, whether it was an ITN, the number of users, and whether it was reported to have been used the previous night. The Roll Back Malaria indicator for the percentage of nets used the previous night (out of all nets observed in the survey) was calculated for all surveys and linear regression was used to assess temporal changes for each type of survey. To evaluate net use in the context of household ITN supply, a variable was created according to ITN supply levels where "not enough" indicated less than 0.5 nets available per person (less than one ITN per two people), “enough” indicated 0.5 to 0.75 nets available per person (at least one ITN per two people), and "more than enough" nets indicated a supply of 0.75 or more nets available per person (i.e. at least two nets per three people). Households consisting of one person with one net were categorized as "enough", rather than "more than enough". For this variable, both untreated nets and ITNs were included.

For the second study objective, all DHS and MIS surveys were reviewed and `r words(length(unique(fulllist)))` surveys from `r words(length(unique(nummis$country)))` countries (Fig. 1B) from `r min(nummis$year)` or later were identified as having included a follow-up question for unused nets, asking for the reason or reasons that the net was not used (in `r words(length(unique(multilist)))` surveys, multiple responses were possible, while in `r words(length(unique(singlelist)))`, only a single response could be selected). `r Words(length(nmis$survey))` were Malaria Indicator Surveys conducted during peak malaria transmission season (approximately three months of fieldwork), and `r words(length(ndhs$survey))` were DHS surveys including the Madagascar 2021 DHS, Nigeria 2018 DHS, the Tanzania 2015-16 DHS/MIS, and eight continuous DHS surveys from Senegal (2011-2019). The DHS surveys also aligned with peak malaria transmission season but were conducted over a longer time period (up to 10 months in Senegal). The "svy" family of commands in Stata 17 was used to appropriately weight results within each country. Plots were produced with R.  

```{r maps, out.width="40%", fig.cap = "A) Number of DHS, MICS, MIS surveys per country included in study objective 1; B) Number of surveys per country containing questions about the reasons nets were not used the previous night"}
# ggpubr::ggarrange(mapa, mapb, label="auto", widths=c(1.8,1), common.legend = T, legend="top")

plot_grid(mapa, mapb, labels="AUTO",ncol=1,align="v", vjust=4, hjust=-5)
```

# Results

The percent of nets used the previous night averaged `r meannetsused`% across all available (n=`r nsurv[1]`) DHS, MIS, and MICS surveys since `r startyr` (Fig. \@ref(fig:fig-netsused)). Linear regression stratified by survey type indicated that there was no significant change over time in the percentage of nets used the previous night for MIS (p=`r round(pnetuseyearmis$p.value[2], digits=3)`), DHS (p=`r round(pnetuseyeardhs$p.value[2], digits=3)`) or MICS (p=`r round(pnetuseyearmics$p.value[2], digits=3)`). MIS surveys, conducted during high transmission season, were associated with a `r round(ptypesurv$estimate[3], digits=1)`-point increase in rates of net use when compared to DHS surveys (p=`r round(ptypesurv$p.value[3], digits=3)`), which are generally conducted during dry season when malaria transmission is lower. Net use rates in MICS surveys did not differ significantly from DHS surveys (p=`r round(ptypesurv$p.value[2], digits=3)`). 

```{r fig-netsused, fig.cap = "Percentage of nets used the night before the survey with linear trend: DHS, MICS, MIS surveys 2003-2020"}
pnets %>% 
  ggplot() +
  ...
  geom_smooth(aes(year, netsused, color=typeofsurvey, fill=typeofsurvey), method="lm", level=0.95, alpha=0.3) +
...
```{r netsused-residence}
pnetsres <- pnets %>% 
  select(country, year, urban, rural, pvalresidence) %>% 
  pivot_longer(cols=c(urban, rural), names_to="residence", values_to = "netsused") %>% 
  mutate(country=case_when(country=="Sierra" ~ "Sierra Leone",
                           country=="Burkina" ~ "Burkina Faso",
                           country=="Congo" ~ "Rep. Congo",
                           country=="Cote" ~ "Cote d'Ivoire",
...
#   ggplot() +
#   #geom_point(aes(year, percent_nets_used), color="gray", alpha=0.5) +
#   geom_text(aes(x=year, y=netsused, label=country, color=residence), size=2, alpha=0.75) +
#   # geom_errorbar(aes(x=year, ymin=netsusedlb, ymax=netsusedub), width=0.3, color="lightgray", alpha=0.5) +
#   geom_smooth(aes(year, netsused, color=residence, fill=residence), method="lm", level=0.95, alpha=0.3) +
#   scale_color_brewer(palette="Set2") +
#   scale_fill_brewer(palette="Set2") +
#   theme(legend.position="bottom") +
#   labs(x="",
...
pnr <- pnets %>% 
         mutate(delta=abs(urban-rural)) %>% 
  arrange(delta) %>% 
  mutate(surveynameyear=fct_reorder(surveynameyear, delta))%>% 
  mutate(country=case_when(country=="Sierra" ~ "Sierra Leone",
                           country=="Burkina" ~ "Burkina Faso",
                           country=="Congo" ~ "Rep. Congo",
                           country=="Cote" ~ "Cote d'Ivoire",
...
# ggplot(pnr) +
#   geom_segment( aes(x=surveynameyear, xend=surveynameyear, y=urban, yend=rural), color="grey") +
#   geom_point( aes(x=surveynameyear, y=urban), color="lightblue", size=2, alpha=.5) +
#   geom_point( aes(x=surveynameyear, y=rural), color="orange", size=2, alpha=.5 ) +
#   coord_flip()+
#   # theme_ipsum() +
#   theme(
...
#   ) +
#   xlab("") +
#   ylab("Percent of bednets used the previous night, urban/rural")
...
pnrdiv <- pnr %>% 
  ...
  #        country!="Cambodia",
  #        country!="Myanmar") %>% 
  mutate(delta=urban-rural) %>% 
  arrange(-delta) %>% 
  mutate(dataset=fct_reorder(dataset, -delta),
         moreurban=if_else(delta>0, "net use higher in urban","net use higher in rural"),
         pvres=if_else(pvalresidence<0.05,1,0),
         moreurban=case_when(pvres==0 ~ "p>0.05",
                             TRUE ~ as.character(moreurban))) 

pnrsum <- pnrdiv %>% 
  tabyl(moreurban) 

# ggplot(pnrdiv) +
#   geom_segment( aes(x=dataset, xend=dataset, y=0, yend=delta, color=as.factor(moreurban), alpha=as.factor(pvres))) +
#   # geom_point( aes(x=surveynameyear, y=urban), color="lightblue", size=2, alpha=.5) +
#   # geom_point( aes(x=surveynameyear, y=rural), color="orange", size=2, alpha=.5 ) +
#   coord_flip()+
#   # scale_color_hue()+
#   scale_alpha_discrete(range=c(0.3, 1), labels=c("p≥0.05", "p<0.05")) +
#   # scale_fill_manual(values=c("#959E4A", "#0091C1")) +
# # theme_ipsum() +
#   theme(
#     # legend.position = "none",
...
#   ) +
#   labs(x="",
#        y="Difference in percent of bednets used the previous night, urban/rural",
#        color="",
#        alpha="")
# ggsave("cleveland_residence_netsused.pdf")



# pnetsres %>% 
#   group_by(residence) %>% 
#   summarize(median=median(netsused))

# pnrdiv %>% 
#   filter(country=="Nigeria") %>% 
#   ggplot() +
#   geom_col(aes(y=delta, x=year, fill=as.factor(moreurban))) + # , alpha=as.factor(pvres))
#   geom_text(aes(y=delta, x=year, label=survey)) +
#     # scale_alpha_discrete(range=c(0.3, 1), labels=c("p≥0.05", "p<0.05")) +
#   # facet_wrap(~country) +
#     # coord_flip()+
#   labs(x="",
...
#   filter(country=="Nigeria") %>% 
#   ggplot() +
#   geom_col(aes(y=abs(delta), x=year, fill=as.factor(moreurban))) + # , alpha=as.factor(pvres))
#   geom_text(aes(y=abs(delta), x=year, label=survey)) +
#     # scale_alpha_discrete(range=c(0.3, 1), labels=c("p≥0.05", "p<0.05")) +
#   # facet_wrap(~country) +
#     # coord_flip()+
#   labs(x="",
...
While the question "Why was this net not used the previous night" was consistent across all surveys, response options were inconsistent between countries and sometimes changed over time within a given country. Table 1 summarizes the response options and categorizes them into seven broad categories. The 'other' category captured 'other' responses (no survey had an "other, specify" option), as well as 'not hung' and 'net not needed last night', as the latter two responses fail to provide useful information about the respondent's reasoning and thus cannot be grouped into other categories.   
...
tzz <- read_excel("/Users/hannahkoenker/Dropbox/A DHS MIS Datasets/Analysis/Reasons/output/Reasons for not using nets.xlsx", sheet="Tanzania2017zonal") %>% 
  ...
  mutate(type = case_when(reason_net_not_used == "net was used the previous night" ~ "net used",
                          ...
                          reason_net_not_used == "net faulty/torn/unusable" ~ "objective",
                          ...
                          grepl("torn", reason_net_not_used) ~ "objective",
                         ...
                         reason_net_not_used == "net too old/torn" ~ "objective",
                          ...
                          reason_net_not_used == "net not needed" ~ "extra",
                         ...
                         reason_net_not_used == "think no need to use" ~ "risk perception",
                          ...
                          reason_net_not_used == "bedbugs" ~ "net attributes",
...
Figure \@ref(fig:summ-reas-cat) below shows the percentage of nets used the previous night across `r words(length(unique(fulllist)))` surveys in `r words(length(unique(nummis$country)))` countries, and the reasons why certain nets were not used. The percentage of nets reported used ranged from 50% in Ghana 2019 to 85% in Mozambique 2018. Senegal's continuous DHS surveys had the highest percentages of reasons relating to risk perception, with up to 25% of nets going unused due to "no mosquitoes" or "no malaria". This category was relatively infrequent in other surveys, except for Tanzania where up to 11% of nets were unused due to risk perception, especially in lower-transmission areas (Supplemental Material, Fig. \@ref(fig:tz-facet)). "Extra/saving for later" nets were reported most frequently in Ghana 2019 (19%), Liberia 2016 (19%), and in Tanzania (12-14% of nets). Senegal and Uganda had the highest rates of 'other' responses. In 2018, Uganda updated response options for this question to be more detailed, resulting in the 'extra' and 'objective' categories becoming more prominent; 'extra' category was comprised largely of 'saving to replace other net', while 'objective' was a combination of 'usual user not here' and 'too old/torn' (Supplemental Material, Fig. \@ref(fig:ug3-reas)). Subsequent surveys in Ghana, Guinea, Mozambique, Madagascar, Mali, and Nigeria adopted similar answer options.


```{r summ-reas-cat, fig.width = 7, fig.height = 5, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Distribution of reasons nets were not used, across surveys. Surveys allowing multiple responses total more than 100% of all nets in the survey."}
...
## C Figure is the above graph but using the residence variable to break each of them out.
urbancat <- x1 %>% reduce(bind_rows) %>%
  drop_na(reason_net_not_used, country) %>% ## drop rows for dfs that aren't relevant (senegal year round, etc)
  select(where( ~ !all(is.na(.)))) %>% ## keep only columns that have some data; drop columns that are all NA
  mutate(reason_net_not_used = tolower(reason_net_not_used)) %>%
  filter(
  ...
  mutate(type = case_when(reason_net_not_used == "net was used the previous night" ~ "net used",
                         ...
                         reason_net_not_used == "don't like size" ~ "net attributes",
                          TRUE ~ as.character(reason_net_not_used))) %>% 
  group_by(country, type) %>% 
  summarize(pcturb=sum(urban),
            pctrur=sum(rural)) 

x5 <- urbancat %>% 
  group_by(type) %>% 
  summarize(pcturb=mean(pcturb),
            pctrur=mean(pctrur)) %>% 
  pivot_longer(cols=!type, names_to = "residence")

surb <- x5 %>% 
  mutate(type=as.factor(type), # make type a factor, for reordering
         type=fct_reorder(type,-value), # order the bars in reverse order of frequency (so they show up in order in plot)
         residence=case_when(residence=="pcturb" ~ "Urban",
                             residence=="pctrur" ~ "Rural")) %>% # order the countries in reverse order  (so they show up in alpha order in plot)
  ggplot() +  
  geom_col(aes(x = value, y=residence, fill=type)) +
  theme_classic() +
  scale_y_discrete(limits=rev) + # reverse order the y-axis so it's in order top to bottom
  scale_x_continuous(breaks=seq(from = 0, to = 120, by = 20), limits=c(0,120)) +
  scale_fill_manual(values = reason_palette) +
...
leg <- get_legend(surb)
surb <- surb + theme(legend.position = "none") 
...
```{r fig-cats, fig.width = 7, fig.height = 5, fig.align='center', fig.topcaption=TRUE, out.width="100%", fig.cap = "Summary of reasons nets were not used the previous night across surveys, by household supply of ITNs"}

(sa + sb)/ (surb + leg) + 
  plot_annotation(tag_levels = list(c("A", "B", "C", "Legend")))
```

Not surprisingly, "extra" nets comprised a higher proportion of reasons for non-use for nets in households with at least two ITN for three people (`r round(x2$value[3], digits=1)`%) compared to nets in households with fewer than one ITN per two people (`r round(x2$value[1], digits=1)`%) or nets in households with at least one ITN for two people but fewer than two nets for three people (`r round(x2$value[2], digits=1)`%). "Other" responses were more frequent for nets in households with at least two ITNs for three people (`r round(x2$value[18], digits=1)`% vs `r round(x2$value[17], digits=1)`% and `r round(x2$value[16], digits=1)`%), indicating that 'other' reasons are likely related to having extra nets, particularly in surveys prior to 2018 when response options did not capture extra nets well. The "risk perception" category was stable across ITN supply categories, ranging between `r round(x2$value[20], digits=1)`% and `r round(x2$value[19], digits=1)`%, as were "subjective" reasons, ranging from `r round(x2$value[23], digits=1)`% to `r round(x2$value[24], digits=1)`%. Reasons for non-use related to net attributes or fears comprised less than 2% of nets across all ITN supply categories. 

```{r}
ngdiff <- pnets %>% 
  filter(country=="Nigeria" & survey!="MICS") %>% 
  mutate(diff=rural-urban) %>% 
  filter(diff>0) %>% 
  select(diff, dataset, year)
```

When stratifying net use by residence (Fig. \@ref(fig:fig-cats)C), there was little difference overall between urban and rural settings, at `r round(x5$value[7], digits=1)`% and `r round(x5$value[8], digits=1)`% of nets used the previous night, respectively. Among the `r count(pnrdiv)` available surveys, `r round(pnrsum$percent[3]*100, digits=0)`% showed no difference in net use by residence, while `r round(pnrsum$percent[1]*100, digits=0)`% had higher rates of net use in rural areas, and `r round(pnrsum$percent[2]*100, digits=0)`% had higher rates in urban areas. Further details on the magnitude of these differences across countries and surveys is provided in Supplemental Material Fig. \@ref(fig:urbrural-bars). 

Rural areas more frequently cited risk perception reasons for non-use, for `r round(x5$value[14], digits=1)`% of nets, compared to urban settings (`r round(x5$value[13], digits=1)`% of nets), but cited subjective reasons less frequently (`r round(x5$value[16], digits=1)`% of nets in rural and `r round(x5$value[15], digits=1)`% of nets in urban areas). More variation was observed in specific countries, however (Supplemental Material, Figs. \@ref(fig:gh-reas)-\@ref(fig:ug3-reas)). The urban/rural discrepancy in percentage of nets used was greatest in Ghana (`r with(pnets, urban[which(dataset=="GHPR82")])`% in urban vs `r with(pnets, rural[which(dataset=="GHPR82")])` in rural areas). Nets in urban areas of Nigeria were also consistently used less than those in rural areas in the four available surveys, declining from a `r with(ngdiff, max(diff))` percentage-point difference in `r with(ngdiff, year[which(diff==max(ngdiff$diff))])` to a `r with(ngdiff, min(diff))` percentage-point difference in `r with(ngdiff, year[which(diff==min(ngdiff$diff))])`. In Kenya, Liberia, Madagascar, Mali, Mozambique, and Uganda, there was virtually no difference, however, and in Tanzania more urban nets were used than rural ones. Results were variable over the nine Senegal surveys. Urban residents in Ghana much more frequently reported "too hot" compared to their rural counterparts (20.7 vs 6.7 in 2019), as did respondents in Nigeria (2015, 2018, and 2021) and Liberia (2016), while the reverse was true in the three Tanzania surveys. Rural residents in Tanzania more frequently reported "no mosquitoes" than those in urban areas in all three surveys; the same was true in Senegal for surveys conducted from 2011-2016, but urban residents cited it more frequently in 2018 and 2021. Urban residents in Ghana (2019) also more frequently reported preferring other methods (for 9.9% of all nets observed), compared to rural areas (4.3% of all nets).
...
```{r seasons-fig, fig.width = 7, fig.height = 5, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Percent of nets used the previous night (green) and reasons for non-use, Senegal 2011-2019"}
...
sreas <- read_excel("/Users/hannahkoenker/Dropbox/A DHS MIS Datasets/Analysis/Reasons/output/Reasons for not using nets.xlsx", sheet="Sen_Season_Reason") %>% 
  ...
  filter(year<2020) %>% # leave out 2020-21 as reasons changed
  rename(ci95=x95_percent_ci,
         reason=reason_net_not_used) %>% 
  mutate(
    # year=case_when(dataset=="SNHR8I" & month==12 ~ 2020,
    #                     TRUE ~ as.numeric(year)),
         reason=str_remove(reason, "reasonnotused=="),
...
sreas %>% 
  ...
  scale_alpha_manual(values=c(0.10,0.0)) +
  labs(x="",
...
Over the past nearly twenty years, an average of over 70% of ITNs were reported as being used the previous night in `r count(pnets)` large household surveys from `r words(length(unique(pnets$country)))` countries. Questions about why nets go unused have only been included in `r words(length(unique(fulllist)))` surveys from `r words(length(unique(nummis$country)))` countries, but among these, the primary reasons given were that unused nets are surplus to immediate requirements or are not needed due to perceived low risk of malaria and/or mosquito bites. Responses related to extra nets were more frequent among households owning more ITNs than deemed strictly necessary by WHO (one ITN per two people) [@WorldHealthOrganization:2019ws]. Unsurprisingly, the proportion of nets used the previous night was lower in households with "more than enough" nets than in households with "not enough" or "enough" nets, while at the same time, the proportion of people that used an ITN the previous night was highest in households with "enough" or "more than enough" ITNs. Households with more than one ITN per two people may have acquired additional nets to cover individual sleeping spaces or to accommodate sleepers who cannot share a sleeping space and thus are able to have most household members sleep under a net; other households may have extra nets being saved for later use, when current nets wear out. Households with "not enough" ITNs had lower rates of population use, but high rates of nets being used - indicating that these households are using the nets that they have, and are challenged primarily by not having enough for other members of the family. It should be noted that having 'extra' nets is reflective of the inherent inefficiencies of ITN distribution systems, wherein some households will have too few while others may receive additional nets slightly earlier than required [@Bhatt:2015gn]. The authors view having extra nets on hand within households as a positive, given the unpredictability of net replacement timing.

Reasons related to net attributes, including size, shape, color, texture, and mosquito-killing ability, were inconsistently included in survey questionnaires, but represented a negligible fraction of reasons for not using nets. While this does not preclude these issues from contributing to net non-use, it provides some evidence that these net attributes are not a key priority when families are making net use decisions. The 2011 Pulford review findings [@Pulford:2011dc] that discomfort due to heat and perceived low mosquito density were the most widely identified reasons for non-use are partially confirmed here; heat per se was not widely reported in more recent surveys, but risk perception as a category, particularly for Senegal, was a key driver. Pulford et al also use categories such as "social factors" (sleeping elsewhere), "technical factors" (not being able to hang a net), which are considered "objective" reasons for non-use in our study. Pulford's review, conducted just as universal coverage campaigns were scaling up, was limited to 22 studies between 1990 and 2010. Since this time, a number of qualitative research studies have also been conducted, in which respondents cite being bothered by net attributes including smell, itching, shape, and size [@Mattern:2016dp; @Berthe:2014ja; @Sande:2012vb; @Galvin:2011ea]. However, these reasons were only rarely cited during quantitative surveys included in this study. Research from Senegal indicates that initial itchiness or smell of nets are transitory, noticeable when nets are first received, but subsiding over time, not impeding net use [@Berthe:2014ja]. Other less preferable attributes of nets may similarly become less noticeable over time, and no longer constitute a key reason for non-use, particularly when, as in most countries distributing ITNs, there are seldom enough nets in good condition for everyone to use. Families thus face choices about using the imperfect ITNs they have, or risk contracting malaria. 
...
Another example of refining the net non-use question to better inform programming is from Uganda. Following the 2009 MIS survey Uganda implemented "hang up campaigns" to ensure nets were hung and used, partially in response to low hanging rates observed in the 2009 and other surveys. Operational research showed that these hang up campaigns did not improve hanging or use rates, as net hanging increased at similar rates over time in control and intervention groups [@Anonymous:2015ff]. In its most recent surveys, Uganda teased apart the nebulous "not hung" answer option to better focus on specific barriers to net use, enabling the programme to understand what lies behind the non-use of nets. Key reasons for non-use in 2018 were "saving net for later", "user not here", and "too old/torn", none of which are best addressed with social behaviour change (SBC) efforts to hang up nets. The specification of reasons for non-use enables programmes and their SBC partners to better design and target net use interventions. The absence of these types of questions even in many recent surveys has been a missed opportunity, particularly as ITNs remain the primary tool for malaria vector control across the globe. Happily, however, the question is now standard in MIS and DHS surveys conducted since 2019, with a set of recommended response options drawn in large part from earlier iterations of the present study. 

Urban and rural differences in net use have been observed in many countries and settings, and typically reflect differences in perception of risk of malaria, driven by underlying malaria transmission rates (which can also vary considerably within urban areas [@10.1186/s12936-016-1138-4]). In Ghana, Guinea, and Nigeria, where malaria risk is considerably lower in urban areas [@nig18DHSreport;@GhanaStatisticalService:2017ug; @ghmis2019; @nigmis2021;@gnmis2021], nets are used at lower rates by urban residents, while in Tanzania, low-transmission zones tend to be more rural, contributing to lower net use rates in rural areas. In the context of lower malaria risk, people are more likely to cite subjective reasons for not using available nets, as the Ghana results demonstrate here (Fig. \@ref(fig:gh-reas) in Supplemental Material).
...
Code is available at https://github.com/hkoenker/Reasons. Data are publicly available from www.dhsprogram.com and from mics.unicef.org.  
...
This work was supported, in whole or in part, by the Bill & Melinda Gates Foundation INV-016322. Under the grant conditions of the Foundation, a Creative Commons Attribution 4.0 Generic License has already been assigned to the Author Accepted Manuscript version that might arise from this submission. The funders had no role in study design, data collection and analysis, decision to publish, or preparation of the manuscript.

## Authors' contributions
HK designed the study and conducted the analysis. EKK and CT contributed to the analysis. ME, RO, ES, EKK, and CT provided inputs on the drafts. All authors reviewed and approved the final version of the manuscript.

## Acknowledgements

An early version of this study was made possible by the generous support of the American people through the United States Agency for International Development (USAID) under the terms of USAID/JHU Cooperative Agreement No: AID-OAAA-14-00057 for the VectorWorks Project. The contents are the responsibility of the authors and do not necessarily reflect the views of USAID or the United States Government. 



# Supplemental Material

```{r summcat-cty, include=FALSE, fig.width=7, fig.height=5, fig.align="center", fig.topcaption=TRUE, out.width="80%", fig.cap = "Reported net use and reasons for non-use equally weighted by country"}
...
ctycat %>% 
  ...
  ggplot() +
  geom_col(aes(x=pct, y=cty, fill=type)) +
  # geom_text(aes(x = 95, y = cty, label=x1), size=3, color="white") +
  scale_fill_manual(values = reason_palette) +
  scale_y_discrete(limits=rev) + # reverse order the y-axis so it's alpha order top to bottom
  labs(y="",
       x="Percent",
...
tzz %>% 
  ...
  group_by(zone, transm, type) %>% 
  summarise(sum=sum(zpct)) %>% 
  ggplot() +
  geom_col(aes(x=type, y=sum, fill=type)) +
  geom_text(aes(x=type, y=sum, label=sum), vjust=-.20, size=3) +
...
```{r urbrural-bars, fig.width=7, fig.height=7, fig.align="center", fig.topcaption=TRUE, out.width="80%", fig.cap = "Percentage-point difference in urban vs rural percent of nets used the previous night, over time"}

pnrdiv %>% 
  ggplot() +
  geom_col(aes(y=delta, x=year, fill=as.factor(moreurban))) +
    # scale_alpha_discrete(range=c(0.3, 1), labels=c("p≥0.05", "p<0.05")) +
  scale_fill_manual(values=c("darkseagreen", "orchid", "lightgrey")) +
  # scale_fill_brewer(palette="Set2") +
  facet_wrap(~country) +
    # coord_flip()+
  labs(x="",
...
```{r func_reasonscombo, eval=FALSE}
...
```{r func_reasons_urban}
### Make a plot for each df:
reasurb <- function(data) {
data %>%
  mutate(reason_net_not_used=tolower(reason_net_not_used)) %>% 
    filter(reason_net_not_used!="total",
           reason_net_not_used!="net used",
  ...
  mutate(type = case_when(reason_net_not_used == "net was used the previous night" ~ "net used",
                         ...
                         reason_net_not_used == "don't like size" ~ "net attributes",
                          TRUE ~ as.character(reason_net_not_used))) %>% 
    pivot_longer(cols=c(urban, rural), names_to="residence") %>% 
  ggplot(aes(x = value, y = fct_reorder(reason_net_not_used, overall), fill = type, alpha=as.factor(residence))) +
  geom_col(position=position_dodge()) +
    scale_alpha_discrete(range=c(.5,1)) +
    scale_x_continuous(limits=c(0,100)) +
    geom_text(aes(x=value, y = fct_reorder(reason_net_not_used, overall), label=value, group=as.factor(residence)), size=3, position=position_dodge(width=.9), hjust=-.2) +
  scale_fill_manual(values = c("net used"=rgb(102,194,165, maxColorValue=255), "objective" = rgb(141,160,203, maxColorValue=255), "subjective" = rgb(252,141,98, maxColorValue=255),  "risk perception" = rgb(231,138,195, maxColorValue=255),  "net attributes" = rgb(229,196,148, maxColorValue=255), "extra" = rgb(166,216,84, maxColorValue=255), "fears" = rgb(255,217,47, maxColorValue=255), "other"="lightgray")) +
  labs(x = "Percent",
       y = "",
       fill = "Reason type",
...
### Ghana
```{r gh-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Ghana 2019"}
reasurb(Ghana2019)  
```

### Guinea

```{r gn-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Guinea 2021"}
reasurb(Guinea2021)
```

### Kenya

```{r kn1-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Kenya 2015"}
reasurb(Kenya2015)
```

```{r kn2-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Kenya 2020"}
reasurb(Kenya2020)
```

### Liberia

```{r lb-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Liberia 2016"}
reasurb(Liberia2016)
```

### Madagascar

```{r md-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Madagascar 2021"}
reasurb(Madagascar2021)
```

### Mali

```{r ml-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Mali 2021"}
reasurb(Mali2021)
```

### Mozambique

```{r mz-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Mozambique 2018"}
reasurb(Mozambique2018)
```

### Nigeria

```{r ng-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Nigeria 2010"}
reasurb(Nigeria2010)
```


```{r ng2-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Nigeria 2015"}
reasurb(Nigeria2015)
```


```{r ng3-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Nigeria 2018"}
reasurb(Nigeria2018)
```


```{r ng4-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Nigeria 2021"}
reasurb(Nigeria2021)
```

### Senegal

```{r sn1-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Senegal 2011"}
reasurb(Senegal2011)
```

```{r sn2-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Senegal 2012"}
reasurb(Senegal2012)
```

```{r sn3-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Senegal 2014"}
reasurb(Senegal2014)
```
 
```{r sn4-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Senegal 2015"}
reasurb(Senegal2015)
```
 
```{r sn5-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Senegal 2016"}
reasurb(Senegal2016)
```
 
```{r sn6-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Senegal 2017"}
reasurb(Senegal2017)
```
  
```{r sn7-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Senegal 2018"}
reasurb(Senegal2018)
```
  
```{r sn8-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Senegal 2019"}
reasurb(Senegal2019)
```
  
```{r sn9-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Senegal 2020-2021"}
reasurb(Senegal2021)
```
 
### Tanzania

```{r tz1-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Tanzania 2011-12"}
reasurb(Tanzania2011)
```
 
 
```{r tz2-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Tanzania 2015"}
reasurb(Tanzania2015)
```

 
```{r tz3-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Tanzania 2017-2018"}
reasurb(Tanzania2017)
```
 
### Uganda

```{r ug1-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Uganda 2009"}
reasurb(Uganda2009)
```

 
```{r ug2-reas, fig.width = 7, fig.height = 7, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Uganda 2014"}
reasurb(Uganda2014)
```
 

```{r ug3-reas, fig.width = 7, fig.height = 7, warning=FALSE, warning=FALSE, fig.align='center', fig.topcaption=TRUE, out.width="80%", fig.cap = "Reasons nets were not used the previous night, Uganda 2019"}
reasurb(Uganda2019)
```
 

```{r teststuff, eval=FALSE}
test <- Ghana2019 %>%
  mutate(reason_net_not_used=tolower(reason_net_not_used)) %>% 
    filter(reason_net_not_used!="total",
           reason_net_not_used!="net used",
